...
function texWood(seed, opt) {
  ...
  for (let b = 0; b < cuts.length - 1; b++) {
    ...
    const pith = x0 + bw * (rnd() * 2.8 - .9);
    const d0 = bw * (.12 + rnd() * 1.7);
...
function texLacquer() {
  ...
  for (let i = 0; i < 40; i++) {
    ...
    path(rx); rx.fillStyle = 'rgba(240,240,240,.75)'; rx.fill();
  }
...
function initGL() {
  ...
  if ('outputColorSpace' in renderer) {
    renderer.outputColorSpace = WANT_POST ? LINEAR_CS : SRGB_CS;
  } else if ('outputEncoding' in renderer) {
    renderer.outputEncoding = WANT_POST ? LINEAR_CS : SRGB_CS;
  }
...
function tx(canvasEl, o) {
  ...
  if (o.srgb !== false) {
    if ('colorSpace' in t) t.colorSpace = SRGB_CS;
    else t.encoding = SRGB_CS;
  }
  t.needsUpdate = true;
  return t;
}
...
function updateWisps(dt) {
  ...
  for (let i = 0; i < N; i++) {
    ...
    w.vx *= 1 - .5 * dt; w.vy = w.vy * (1 - .5 * dt) + .022 * dt;
...
const CAM = [
  ...
  { p: [  0.0, 7.60, -16.0 ], t: [  0.0, 13.0, -40.0 ], fov: 42 },
  { p: [  0.0, 10.5, -20.0 ], t: [  0.0, 3.00, -34.0 ], fov: 46 }
];
...
return {
  ...
  setScrollProgress(progress, maxSections = 5) {
    RIG.prog = progress;
    RIG.smooth = clamp(progress * maxSections, 0, maxSections);
  },
  dispose,
};
}
